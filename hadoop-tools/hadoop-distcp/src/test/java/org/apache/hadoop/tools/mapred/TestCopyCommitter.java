...
public class TestCopyCommitter {
  ...
  @Test
  public void testIgnoreDeletedInCommitter() {
    TaskAttemptContext taskAttemptContext = getTaskAttemptContext(config);
    JobContext jobContext = new JobContextImpl(taskAttemptContext.getConfiguration(),
        taskAttemptContext.getTaskAttemptID().getJobID());
    Configuration conf = jobContext.getConfiguration();
    ...
    try {
      ...
      fs.mkdirs(new Path(sourceBase + "/newTest/testIgnoreDeletedInCommitter"));
      TestDistCpUtils.createFile(fs, new Path(sourceBase, "/newTest/testIgnoreDeletedInCommitter/1"));
      fs.setPermission(new Path(sourceBase + "/newTest/testIgnoreDeletedInCommitter"), initialPerm);

      fs.mkdirs(new Path(targetBase + "/newTest/testIgnoreDeletedInCommitter"));
      TestDistCpUtils.createFile(fs, new Path(targetBase, "/newTest/testIgnoreDeletedInCommitter/1"));
      fs.setPermission(new Path(targetBase + "/newTest/testIgnoreDeletedInCommitter"), initialPerm);
      ...
      conf.set(DistCpConstants.CONF_LABEL_TARGET_WORK_PATH, targetBase);
      conf.setBoolean(DistCpConstants.CONF_LABEL_IGNORE_DELETED, true);
      fs.delete(new Path(targetBase + "/newTest/testIgnoreDeletedInCommitter"), true);
