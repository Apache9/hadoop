...
class RpcDuplexHandler extends ChannelDuplexHandler {
  ...
  private void writeRequest(ChannelHandlerContext ctx, Call call,
    ...
    RpcRequestHeaderProto rpcHeader = ProtoUtil.makeRpcRequestHeader(
        RpcKind.RPC_PROTOCOL_BUFFER, OperationProto.RPC_FINAL_PACKET, call.id,
        0, conn.rpcClient.clientId);
    int rpcHeaderSize = rpcHeader.getSerializedSize();
    RequestHeaderProto requestHeader =
        RequestHeaderProto.newBuilder().setMethodName(call.methodName)
            .setDeclaringClassProtocolName(call.protocolName)
            .setClientProtocolVersion(call.protocolVersion).build();
    ...
    int totalSize = CodedOutputStream.computeRawVarint32Size(rpcHeaderSize) +
        ...
        requestHeaderSize;
    if (call.param != null) {
      int paramSize = call.param.getSerializedSize();
      totalSize +=
    ...
    if (call.param != null) {
      call.param.writeDelimitedTo(out);
    }
  ...
  private void readResponse(ChannelHandlerContext ctx, ByteBuf buf)
    ...
    Call call = id2Call.remove(id);
    call.setResponse(
        call.responseDefaultType.getParserForType().parseDelimitedFrom(in));
  }
