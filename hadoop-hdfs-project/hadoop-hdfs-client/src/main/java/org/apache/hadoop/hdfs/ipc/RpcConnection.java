...
class RpcConnection {
  ...
  private void writeConnectionContext(Channel ch) throws IOException {
    RpcRequestHeaderProto connectionContextHeader =
        ProtoUtil.makeRpcRequestHeader(RpcKind.RPC_PROTOCOL_BUFFER,
            ...
            RpcConstants.INVALID_RETRY_COUNT, rpcClient.clientId);
    int headerSize = connectionContextHeader.getSerializedSize();
    IpcConnectionContextProto message = ProtoUtil.makeIpcConnectionContext(
        remoteId.protocolName, remoteId.ticket, AuthMethod.SIMPLE);
  ...
  private Channel connect() {
    ...
    channel = new Bootstrap().group(rpcClient.group)
        .channel(rpcClient.channelClass).option(ChannelOption.TCP_NODELAY, true)
        .option(ChannelOption.SO_KEEPALIVE, true)
        .handler(new BufferCallBeforeInitHandler())
        .remoteAddress(remoteId.address).connect()
