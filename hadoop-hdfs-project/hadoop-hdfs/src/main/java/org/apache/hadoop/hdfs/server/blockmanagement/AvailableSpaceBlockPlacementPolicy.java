...
public class AvailableSpaceBlockPlacementPolicy extends BlockPlacementPolicyDefault {
  ...
  public void initialize(Configuration conf, FSClusterStats stats, NetworkTopology clusterMap) {
    ...
    if (balancedPreferencePercent < 0.5) {
      LOG.warn("The value of "
          + DFS_NAMENODE_AVAILABLE_SPACE_BLOCK_PLACEMENT_POLICY_BALANCED_SPACE_PREFERENCE_FRACTION_KEY
          + " is less than 0.5 so volumes with less remaining datanode will receive more block allocations");
    }
  }
  ...
  protected DatanodeDescriptor chooseDataNode(String scope) {
    ...
    if (ret == 0) {
      ...
      return (RAND.nextFloat() < balancedPreferencePercent) ? a : b;
    } else {
      return (RAND.nextFloat() < balancedPreferencePercent) ? b : a;
    }
  }
