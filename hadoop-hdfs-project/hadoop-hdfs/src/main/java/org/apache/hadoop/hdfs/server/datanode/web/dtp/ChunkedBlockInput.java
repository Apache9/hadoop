...
class ChunkedBlockInput implements ChunkedInput<Http2DataFrame> {

  private static final Log LOG = LogFactory.getLog(ChunkedBlockInput.class);
  ...
  public ChunkedBlockInput(InputStream blockInput, InputStream checksumInput,
      byte[] lastChunkChecksum, DataChecksum checksum, int chunksPerFrame,
      long length) {
  ...
  public Http2DataFrame readChunk(ByteBufAllocator allocator)
    ...
    if (checksumInput != null) {
      ...
      if (isEndOfInput() && lastChunkChecksum != null) {
        ...
        System.arraycopy(lastChunkChecksum, 0, blockReadBuffer,
            checksumLength - checksum.getBytesPerChecksum(),
    ...
    OpReadBlockFrameHeaderProto header =
        OpReadBlockFrameHeaderProto.newBuilder()
            .setNumChunks(numberOfBlockChunks(blockBytesToRead,
                checksum.getBytesPerChecksum()))
            .setChecksums(checksums).setDataLength(blockBytesToRead).build();
    ...
    return new DefaultHttp2DataFrame(data, isEndOfInput());
  }
