...
public class ServerHttp2EventListener extends Http2EventAdapter {
  ...
  private static final Http2FrameLogger FRAME_LOGGER = new Http2FrameLogger(
      LogLevel.INFO, ServerHttp2EventListener.class);
  ...
  public void onStreamAdded(final Http2Stream stream) {
    ServerHttp2StreamChannel subChannel =
        new ServerHttp2StreamChannel(parentChannel, stream);
  ...
  private ServerHttp2StreamChannel getSubChannel(int streamId)
    ...
    if (streamId == CONNECTION_STREAM_ID) {
      conn.forEachActiveStream(new Http2StreamVisitor() {
        ...
        public boolean visit(Http2Stream stream) throws Http2Exception {
          ServerHttp2StreamChannel subChannel =
              stream.getProperty(subChannelPropKey);
          if (subChannel != null) {
            subChannel.tryWrite();
          }
          return true;
  ...
  public static Http2ConnectionHandler
      create(Channel channel,
          ChannelInitializer<ServerHttp2StreamChannel> initializer,
          boolean verbose) {
