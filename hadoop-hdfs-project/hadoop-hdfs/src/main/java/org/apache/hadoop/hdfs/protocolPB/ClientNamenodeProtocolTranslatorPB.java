...
public class ClientNamenodeProtocolTranslatorPB implements
  ...
  public void refreshTopology() throws IOException {
    RefreshTopologyRequestProto req = RefreshTopologyRequestProto.newBuilder().build();
    try {
      rpcProxy.refreshTopology(null, req);
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  }
  ...
  @Override
  public QuotaSummary getQuotaSummary(String path)
      throws AccessControlException, FileNotFoundException,
      UnresolvedLinkException, IOException {
    GetQuotaSummaryRequestProto req = GetQuotaSummaryRequestProto
        .newBuilder()
        .setPath(path)
        .build();
    try {
      return PBHelper.convert(rpcProxy.getQuotaSummary(null, req)
          .getSummary());
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  ...
  public void removeDefaultAcl(String src) throws IOException {
    ...
    try {
      ...
      if (TracerLog.isClientTracing()) {
        Trace.addTimelineAnnotation("HDFS: HdfsPerfCounter done. src=" + src);
      }
    }
  }
