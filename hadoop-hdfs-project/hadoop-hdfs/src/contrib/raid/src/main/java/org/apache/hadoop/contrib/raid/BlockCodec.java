...
public class BlockCodec {
  ...
  public void encode(Path file) throws IOException {
    ...
    FSDataOutputStream codingOut = fs.create(tmpCodingFile, true,
        ...
        fs.getDefaultReplication(tmpCodingFile), fileStatus.getBlockSize());

    // Encode the blocks
    int roundNum = (locations.length - 1) / dataBlocksNum + 1;
    for (int r = 0; r < roundNum; ++r) {
      encodeBlocks(file, fileStatus, locations.length, codingOut,
          r * dataBlocksNum, Math.min(dataBlocksNum,
              locations.length - r * dataBlocksNum));
    }
  ...
  public void decode(Path file, int[] corruptedBlocks) throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < groupNum; ++i) {
        decodeBlocks(file, fileStatus, locations.length, i, blockSize,
            erasuredDataInfos[i], erasuredCodingInfos[i]);
      }
    } finally {
  ...
  public byte[] decode(Path file, long offset, int length) throws IOException {
    ...
    try {
      ...
      decodeData(file, fileStatus, locations.length, groupNo, blockSize,
          adjustedOffset, adjustedLength, erasuredDataInfo, erasuredCodingInfo);
    ...
    try {
      ...
      for (int r = 0; r < blockSize / STRIPE_SIZE; ++r) {
        ...
        for (int i = 0; i < dataIns.length; ++i) {
          ...
          if (isLastGroup && (i == dataIns.length - 1) &&
        ...
        for (int i = 0; i < coding.length; ++i) {
          System.arraycopy(coding[i], 0, result[i], r * STRIPE_SIZE,
              coding[i].length);
        }
      }
   ...
   */
  void decodeBlocks(Path file, FileStatus fileStatus, int totalBlockNum,
      int groupNo, long blockSize, Map<Integer, OutputStream> dataErasures,
      Map<Integer, OutputStream> codingErasures) throws IOException {
    decodeData(file, fileStatus, totalBlockNum, groupNo, blockSize, 0,
        blockSize, dataErasures, codingErasures);
  }

  void decodeData(Path file, FileStatus fileStatus, int totalBlockNum,
      int groupNo, long blockSize, long offset, long length,
    ...
    try {
      ...
      for (int r = 0; r < length / STRIPE_SIZE; ++r) {
        // Read next stripe of data
        int count = readDataStripe(file, fileStatus, totalBlockNum, groupNo,
            offset, r, dataErasures, dataIns, data);

        // Read next stripe of coding
        readCodingStripe(file, groupNo, blockSize, offset, r,
            codingErasures, codingIns, coding, count);

        // Decode the corrupted data/coding and write back to the file
        codec.decode(erasures, data, coding);
        writeDecodedData(fileStatus, totalBlockNum, groupNo, offset, r,
            dataErasures, codingErasures, data, coding);
      }
    } finally {
      closeStreams(dataIns);
  ...
  int readDataStripe(Path file, FileStatus fileStatus, int totalBlockNum,
      int groupNo, long offset, int roundNo,
      Map<Integer, OutputStream> dataErasures, FSDataInputStream[] dataIns,
      ...
      if (blockIdx < totalBlockNum && dataErasures.get(blockIdx) == null) {
        try {
          ...
          if (isBlockCorrupted(e)) {
            dfsClient.reportBadBlocks(
                new LocatedBlock[] {getBlock(file, blockSize, blockIdx)});
          }
          throw e;
        }
        ++readStripeCount;
  ...
  int readCodingStripe(Path file, int groupNo, long blockSize, long offset,
      int roundNo, Map<Integer, OutputStream> codingErasures,
      FSDataInputStream[] codingIns, byte[][] coding, int dataStripeCount)
      throws IOException {
    int readStripeCount = 0;
    for (int i = 0; i < codingBlocksNum; ++i) {
      int blockIdx = groupNo * codingBlocksNum + i;
      if (readStripeCount < dataBlocksNum - dataStripeCount &&
          codingErasures.get(blockIdx) == null) {
        try {
          long pos = blockIdx * blockSize + offset + roundNo * STRIPE_SIZE;
          codingIns[i].read(pos, coding[i], 0, coding[i].length);
        } catch (IOException e) {
          if (isBlockCorrupted(e)) {
            dfsClient.reportBadBlocks(new LocatedBlock[] {
                getBlock(getCodingFile(file), blockSize, blockIdx)});
          }
          throw e;
        }
        ++readStripeCount;
  ...
  void writeDecodedData(FileStatus fileStatus, int totalBlockNum, int groupNo,
      long offset, int roundNo, Map<Integer, OutputStream> dataErasures,
  ...
  LocatedBlock getBlock(Path file, long blockSize, int blockIdx)
      throws IOException {
