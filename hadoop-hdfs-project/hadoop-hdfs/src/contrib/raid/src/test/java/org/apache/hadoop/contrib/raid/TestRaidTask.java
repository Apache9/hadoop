...
public class TestRaidTask {
  ...
  public static void setUpClass() throws Exception {
    ...
    conf.setInt(DFSConfigKeys.DFS_DATANODE_BALANCE_MAX_NUM_CONCURRENT_MOVES_KEY, 100);
  ...
  private void checkBlockPlacement(Path file) throws Exception {
    ...
    // It turns out that the sweeper is not definitely scheduled before mover test case
    int expectedScannedFiles = 8;
    if (dfs.exists(new Path("/0/1/f"))) {
    	expectedScannedFiles = 9;
    }
    Assert.assertEquals(rd.getMetrics().filesScannedForMover.value(), expectedScannedFiles);
