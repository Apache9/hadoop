...
public class TestReplicationPolicyOverused {
  ...
  private static final long overUsedFreespaceThreshold =
      MIN_BLOCKS * BLOCK_SIZE + 3 * BLOCK_SIZE;
  ...
  public static void setupCluster() throws Exception {
    ...
    for (int i=0; i < NUM_OF_DATANODES; i++) {
      updateHeartbeatWithUsage(dataNodes[i],
          2*MIN_BLOCKS*BLOCK_SIZE, 0L,
          2*MIN_BLOCKS*BLOCK_SIZE, 0L, 0L, 0L, 0, 0);
    }    
  }
  ...
  public void testChooseTargetWithOverusedNodes() throws Exception {
    ...
    // freespace
    long capacity = 13 * MIN_BLOCKS * BLOCK_SIZE;
    long dfsUsed = 9 * MIN_BLOCKS * BLOCK_SIZE;
    ...
    capacity = 3 * MIN_BLOCKS * BLOCK_SIZE;
    ...
    // Condition 3: Overused from freespace point of view but not percentage
    capacity = 4 * MIN_BLOCKS * BLOCK_SIZE;
    ...
    updateHeartbeatWithUsage(dataNodes[0], 2
        * MIN_BLOCKS * BLOCK_SIZE, 0L, 2
        * MIN_BLOCKS * BLOCK_SIZE, 0L, 0L, 0L, 0, 0);
  ...
  public void testChooseTargetWithHalfOverusedNodes() throws Exception {
    ...
    long capacity = 4 * MIN_BLOCKS * BLOCK_SIZE;
