...
public class ZookeeperConfigurationService extends ConfigurationService {
  ...
  // ZookeeperConfigurationService config items
  public static final String CONFIGURATION_SERVICE_ZOOKEEPER_HOST =
      "configuration.service.zookeeper.host";
  public static final String CONFIGURATION_SERVICE_ZOOKEEPER_SESSION_TIMEOUT =
  ...
  public ZookeeperConfigurationService(Configuration conf) {
    super(conf);
  }

  @Override
  public Configuration fetchConfiguration(String authority) throws IOException {
    String zkHost = conf.get(CONFIGURATION_SERVICE_ZOOKEEPER_HOST);
    if (zkHost == null) {
      throw new IOException("Couldn't find Zookeeper Host.");
    }
    String timeout = conf.get(CONFIGURATION_SERVICE_ZOOKEEPER_SESSION_TIMEOUT);
