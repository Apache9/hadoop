...
public class ContentSummary implements Writable{
   ...
   */
  private static final String STRING_FORMAT = "%12s %12s %18s %18s ";
  /** 
   * Output format:
   * <----12----> <----15----> <----15----> <----15----> <----12----> <----12----> <-------18-------> <-------18------->
   *    QUOTA   REMAINING_QUATA SPACE_QUOTA SPACE_QUOTA_REM DIR_COUNT   FILE_COUNT   CONTENT_SIZE       SPACE_CONSUMED   FILE_NAME
   */
  ...
  private static final String HEADER = String.format(
      STRING_FORMAT, "directories", "files", "bytes", "consumed space");
  ...
  */
  ...
  public String toString(boolean qOption, boolean hOption) {
    ...
    return prefix + String.format(STRING_FORMAT,
     ...
     formatSize(spaceConsumed, hOption));
  }
  /**
